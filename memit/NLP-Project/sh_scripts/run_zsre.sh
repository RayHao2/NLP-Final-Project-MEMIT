...
RUN_GROUP="qwen-memit-zsre-${timestamp}"
RUN_GROUP_DIR="$RESULTS_DIR/$RUN_GROUP"
MASTER_LOG="$RUN_GROUP_DIR/zsre_staged_eval_${timestamp}.log"
...
{
  echo "Qwen MEMIT zsRE staged evaluation"
  echo "Started: $(date)"
  ...
  run_zsre_stage "stage1_zsre_100" 100
  run_zsre_stage "stage2_zsre_500" 500
  run_zsre_stage "stage3_zsre_1000" 1000

  echo ""
  echo "All zsRE stages finished: $(date)"
} 2>&1 | tee "$MASTER_LOG"
